...
export default function HomeScreen({ navigation }) {
  ...
  // Servicio activo más reciente — excluye finalizado y rechazado
  const activeService = useMemo(() =>
    services.find(s => s.status !== 'finalizado' && s.status !== 'rechazado'),
    [services]
  );
          ...
          <View style={styles.activeBannerLeft}>
            ...
            <View style={{ marginLeft: 10 }}>
              ...
              <Text style={styles.activeBannerSub} numberOfLines={1}>
                {activeService.professionalName || 'Profesional'} · {STATUS_LABEL[activeService.status] || activeService.status}
              </Text>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  // Banner
  activeBanner:      { backgroundColor: COLORS.primary, borderRadius: 14, padding: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 },
