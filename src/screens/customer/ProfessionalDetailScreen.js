...
export default function ProfessionalDetailScreen({ route, navigation }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={common.hintText}>{professional.category} • {Number(professional.rating).toFixed(1)} ★</Text>
...
const styles = StyleSheet.create({
  container: { 
    flexGrow: 1, backgroundColor: COLORS.background, alignItems: 'center', padding: 20, paddingBottom: 40 
  },
  name: { 
    color: COLORS.textMain, fontSize: 20, fontWeight: '600', marginTop: 12, textAlign: 'center' 
  },
  jobTitleBadge: { 
    backgroundColor: COLORS.inputBg, borderRadius: 999, paddingHorizontal: 14, paddingVertical: 4, marginTop: 6, borderWidth: 1, borderColor: COLORS.border 
  },
  jobTitleText: { 
    color: COLORS.primaryDark, fontWeight: '600', fontSize: 13 
  },
  sectionTitle: { 
    alignSelf: 'flex-start', color: COLORS.textMain, fontWeight: '600', marginTop: 24, marginBottom: 6 
  },
  description: {
     alignSelf: 'flex-start', color: COLORS.textSecondary, fontSize: 14, lineHeight: 22, marginBottom: 24 
    },
});